...
from .serializers import (
    ...
    BlogsCategorySerializer, TagSerializer
)
...
class PostListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_authenticated or not user_has_permission(self.request.user, 'publish_blog'):
    ...
    def post(self, request, *args, **kwargs):
        if not (request.user.is_authenticated and user_has_permission(request.user, 'publish_blog')):
            return Response({'detail': 'You do not have permission to publish blog posts.'}, status=status.HTTP_403_FORBIDDEN)
...
class PostDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if post.author != request.user:
            return Response({'detail': 'Only the author can update this post.'}, status=status.HTTP_403_FORBIDDEN)
...
def blog_stats(request: Request) -> Response:
    ...
    stats = {
        ...
        'total_comments': Comment.objects.filter(content_type=ctype, is_approved=True).count(),
        'total_ratings': Rating.objects.filter(content_type=ctype).count(),
    }
